...
export function calcIndicators(candles: Candle[]): Indicators {
  ...
  return {
    ...
    macd: Number(lastMacd?.MACD ?? 0),
    macdSignal: Number(lastMacd?.signal ?? 0),
    macdHistogram: Number(lastMacd?.histogram ?? 0),
    rsi14: Number(rsiArr[rsiArr.length - 1] ?? 50),
    bbUpper: Number(lastBB?.upper ?? 0),
    bbMiddle: Number(lastBB?.middle ?? 0),
    bbLower: Number(lastBB?.lower ?? 0),
    lastClose,
